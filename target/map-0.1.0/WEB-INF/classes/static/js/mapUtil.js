~(function(L,map) {
    ...
    mapUtil.task=function(){
            ...
            if(paths instanceof Array){
                for(var i=0;i<paths.length;i++){
                    for(var j=0;j<newtoGO.length;j++){
                        //console.log(paths[i].content.road.id+"----");
        ...
        function checkTask(){
            ...
            if(input.length<3||input[1]==input[2]){
                console.log("新建任务参数不对");
                return null;
            }
            for(var i=0;i<taskList.length;i++){
                if(input[0]==taskList[i].id){
                    console.log("任务id重复");
                    return null;
                }
            }
    ...
    mapUtil.act=function(){
        ...
        function regisCar(){
            ...
            if(input.length>Object.getOwnPropertyNames(car.info).length){
                return alert("将要注册的 "+input[0]+" 车辆属性个数超出限制");
            }
            if(findCarByName(input[0])!=null){
                return alert("将要注册的车辆名称重复");
            }
            var i=0;
            for(var prop in car.info){
                car.info[prop]=input[i++];
            }
    	...
    	return{
            ...
            runTask:function(d1,d2){
                console.log(map.distance(d1,d2));
